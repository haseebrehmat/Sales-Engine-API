...
def upload_jobs_in_sales_engine_staging(jobs_data, filename=None):
    try:
        ...
        url = 'https://decagon-staging.devsinc.com/job_portal/api/v1/job_roles'
...
resp = requests.get('https://decagon-staging.devsinc.com/job_portal/api/v1/job_roles', headers=headers)


# keywords_dict = {k.lower().replace(" ", ""): v for k, v in keyword.items()}
# print(keywords_dict)
# keywords = [x.lower() for x in set(JobDetail.objects.values_list('tech_keywords', flat=True))]
